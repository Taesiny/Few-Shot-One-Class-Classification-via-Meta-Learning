...
#
        ...
        self.X_outer_loop = tf.compat.v1.placeholder(
            tf.compat.v1.float32, (None, self.dim_input), name='X_outer_loop')
        self.Y_outer_loop = tf.compat.v1.placeholder(
            tf.compat.v1.float32, (None, self.n_classes), name='Y_outer_loop')
        ...
        self.inner_loop_optimizer = tf.compat.v1.train.GradientDescentOptimizer(
            self.lr)
        self.finetune_update_op = self.inner_loop_optimizer.minimize(
            self.finetune_loss)
        for i in range(1, self.num_updates):
            if(i == 1):
                with tf.compat.v1.control_dependencies([self.finetune_update_op]):
                ...
                with tf.compat.v1.control_dependencies([finetune_update_op]):

                    finetune_output = self.construct_forward(
                        self.X_finetune, training=True)
            finetune_loss = tf.compat.v1.reduce_mean(
                self.loss_fct(
                    labels=self.Y_finetune,
                    logits=finetune_output))
        ...
        if(self.bn):
            ...
            with tf.compat.v1.control_dependencies([finetune_update_op]):
                ...
                with tf.compat.v1.control_dependencies([self.updated_bn_model]):
        ...
        self.outer_loop_loss = tf.compat.v1.reduce_mean(
            self.loss_fct(
                labels=self.Y_outer_loop,
                logits=self.outer_loop_output))

        self.meta_optimizer = tf.compat.v1.train.AdamOptimizer(self.meta_lr)
        ...
        for grad_var in self.meta_gradient:
            self.placeholder_gradients.append(
                (tf.compat.v1.placeholder(tf.compat.v1.float32, shape=grad_var[0].get_shape()), grad_var[1]))
        ...
        val_finetune_output = self.construct_forward(
            self.X_finetune, training=True)
        val_finetune_loss = tf.compat.v1.reduce_mean(
            self.loss_fct(
                labels=self.Y_finetune,
    ...
    def compute_grad(self, X, Y, test):
        ...
        if(test):
            gradient_for_batch = tf.compat.v1.gradients(self.test_loss, self.m_vars)
        else:
            gradient_for_batch = tf.compat.v1.gradients(self.finetune_loss, self.m_vars)
    ...
    def metatrain_op(self, epoch, X_train_a, Y_train_a, X_train_b, Y_train_b):
        ...
        train_summaries = tf.compat.v1.Summary(
            value=[
